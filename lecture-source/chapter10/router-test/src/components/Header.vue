<template>
  <nav class="navbar navbar-expand-md bg-dark navbar-dark mt-2">
    <span class="navbar-brand">이날치(LeeNalChi)</span>
    <button class="navbar-toggler" type="button" @click="changeIsNavShow">
      <span class="navbar-toggler-icon"></span>
    </button>

    <div :class="navClass">
      <ul class="navbar-nav">
        <li class="nav-item">
          <!-- router-link: 라우터 링크 태그 -->
          <!-- to 속성에 이동할 경로 지정 -->
          <router-link class="nav-link" to="/">홈</router-link>
        </li>
        <li class="nav-item">
          <router-link class="nav-link" to="/about">소개</router-link>
        </li>
        <li class="nav-item">
          <router-link class="nav-link" to="/members">멤버</router-link>
        </li>
        <li class="nav-item">
          <router-link class="nav-link" :to="{ name: 'vi' }">영상1</router-link>
        </li>
        <li class="nav-item">
          <router-link class="nav-link" :to="{ name: 'vi' }">영상2</router-link>
        </li>
        <li class="nav-item">
          <router-link class="nav-link" :to="{ name: 'vi' }">영상3</router-link>
        </li>

        <li class="nav-item">
          <RouterLink class="nav-link" to="/test">테스트</RouterLink>
        </li>
      </ul>
    </div>
  </nav>
</template>

<script>
import { reactive, computed } from 'vue';

export default {
  setup() {
    const state = reactive({
      isNavShow: false,
    });
    const navClass = computed(() =>
      state.isNavShow
        ? 'collapse navbar-collapse show'
        : 'collapse navbar-collapse'
    );
    const changeIsNavShow = () => {
      state.isNavShow = !state.isNavShow;
    };

    return { state, navClass, changeIsNavShow };
  },
};
</script>
