...
<template>
  <div>
    <h3>Compostion API</h3>
    <input
      type="text"
      placeholder="입력하면 대문자로 변환"
      :value="name"
  ...
  <div>
    ...
    <button @click="calculateSum(x, y)">계산하기</button>
    <p>X + Y = {{ sum1 }}</p>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    // setup() return 구문에 작성 -> <template>에서 사용 가능
    return { name, changeName, x, y, sum1, calculateSum };
  },
};
</script>
